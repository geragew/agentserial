...
from agentserial.models import CheckResult, Contract, History, VerdictStatus


def generate_report(history: History, contract: Contract, result: CheckResult) -> str:
    status_class = "pass" if result.status == VerdictStatus.ROBUST_PASS else "warn" if result.status in {
    ...
    diagnostics = "".join(f"<li>{escape(item)}</li>" for item in violations) or "<li>No violation diagnostic.</li>"
...
<header><div class="brand"><svg class="mark" viewBox="0 0 32 32"><path d="M3 7h8l5 7 5-7h8M3 25h8l5-7 5 7h8"/><path class="x" d="M13 16h6"/></svg>AgentSerial</div><small>LOCAL REPORT / v0.1.0</small></header>
...
def _operation_summary(operation: object) -> str:
    reads = getattr(operation, "reads")
